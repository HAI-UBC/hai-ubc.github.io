...
if(isset($_POST['submitted'])){
	...
	if(!$missing_fields && isset($_POST['uid'])){
		...
		print "Done";
		header('Location: http://hai.cs.ubc.ca');
	}
}
...
<form id="testform" class="appnitro" action="NC.php<?php if(isset($_GET['uid'])) print "?uid=".addslashes(htmlentities($_GET['uid']));  ?>" method="POST">
	...
	<!--  
	Modified by Genaro on 6th April 2023 to allow UID reading 
	-->
    <ul id = 'questionList'>

    <div>
		<label>User ID: 
			<input type="text" name="uid" value="<?php  if(isset($_GET['uid'])) echo addslashes(htmlentities($_GET['uid'])); ?>" />
			<b><?php if(isset($_GET['uid'])) {print addslashes(htmlentities($_GET['uid'])); $_POST['uid'] = (htmlentities($_GET['uid']));} ?></b>
		</label>
        <p>
